...
function SpriteBatch() {
    this.super(this.constructor, 'constructor', arguments);
}
util.inherits(SpriteBatch, DisplayObject);

SpriteBatch.prototype.init = function() {
    this.obj = new Phaser.SpriteBatch(this.game);
    this.super(this.constructor, 'init');
};

SpriteBatch.prototype.update = function(prevProps) {
    var event;
    if (!this.obj)
        return;
    ...
    for (var i = 0; i < events.length; i++) {
        ...
        if (event in this.props) {
            if (prevProps && event in prevProps)
                this.obj[event].remove(prevProps[event]);
            this.obj[event].add(this.props[event]);
        }
    }
    this.super(this.constructor, 'update');
};

Object.defineProperties(SpriteBatch.prototype, {
